...
from utils import generate_pinyin
...
def process_novel(novel_dir, start_chapter: int, stop_event: threading.Event, redo_pinyin: bool = False):
    ...
    # List to hold all chapter decks for the final master package
    all_chapter_decks = [] 
    ...
    for chapter in chapters:
        ...
        # Logic: If everything exists, we skip (UNLESS we are in redo-pinyin mode)
        is_complete = (
            xhtml_path.exists() and 
            ch_apkg_path.exists() and 
            consolidated_json.exists() and 
            existing_audio == total_lines
        )

        if is_complete and not redo_pinyin:
        ...
        if redo_pinyin and consolidated_json.exists():
            ...
            for line in data:
                if "cn" in line:
                    line["py"] = generate_pinyin(line["cn"])
            consolidated_json.write_text(json.dumps(data, ensure_ascii=False, indent=4), encoding='utf-8')
            # We don't need to reload chapter_lines here, the next block will do it
        ...
        # 1. Load full translation if available
        ...
        # 2. Else, process chunks
            ...
            for i, chunk_dict in enumerate(chunks):
                ...
                # Update Glossary (With Dirty Flag)
                try:
                ...
                # Pinyin (Local)
                py_dict = {}
                for idx, text_line in chunk_dict.items():
                    py_dict[idx] = generate_pinyin(text_line)
        ...
        # --- VRAM BRIDGE ---
        if not redo_pinyin: # Only unload if we plan to use TTS
        ...
        # Subdeck Creation (Novel::Chapter)
        ...
        for line_idx, line in enumerate(chapter_lines):
            ...
            # --- GENERATION BLOCK ---
            # If redo_pinyin is True, we SKIP this block entirely but keep the path logic below
            if not skip_audio and not redo_pinyin:
                ...
                if torch.is_tensor(wavs[0]):
                    ...
                    if audio_tensor.numel() == 0 or torch.isnan(audio_tensor).any() or torch.isinf(audio_tensor).any():
                        ...
                        audio_data = (audio_tensor.detach().cpu().to(torch.float32).contiguous().numpy().copy())
                else:
        ...
        # 1. Add this chapter deck to the master list
        all_chapter_decks.append(chapter_deck)

        # 2. Export INDIVIDUAL Chapter Package
        ...
        # 5. Export MASTER Anki Package (All subdecks)
