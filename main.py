...
from utils import (
    Chapter,
    call_llm,
    chunk_text_into_numbered_lines,
    clean_for_tts,
...
def run_text_stage(chapter, paths, glossary, stop_event, redo_pinyin):
    ...
    for i, chunk_dict in enumerate(chunks):
        ...
        if chunk_cache_file.exists():
            ...
            chapter_lines.extend(
                json.loads(chunk_cache_file.read_text(encoding="utf-8"))
            )
            continue

        print(
            f"    - Chunk {i+1}/{len(chunks)} ({len(chunk_dict)} lines): Sending to LLM..."
        )
        numbered_input = "\n".join(
        ...
        try:
            ...
            if glossary_changed:
                paths["glossary"].write_text(
                    json.dumps(glossary, ensure_ascii=False, indent=4), encoding="utf-8"
                )
        except Exception:
            pass
        ...
        chunk_cache_file.write_text(
            json.dumps(current_chunk_lines, ensure_ascii=False), encoding="utf-8"
        )
    ...
    if not stop_event.is_set() and len(chapter_lines) == total_lines:
        ...
        consolidated_json.write_text(
            json.dumps(chapter_lines, ensure_ascii=False, indent=4), encoding="utf-8"
        )
...
def run_audio_stage(chapter, chapter_lines, novel_name, paths, stop_event, redo_pinyin):
    ...
    chapter_deck = genanki.Deck(
        chapter_deck_id, f"{safe_deck_title}::Ch {chapter.chapter_number:03d}"
    )
    ...
    full_text_en = ""
    epub_body = (
        f"<h1>{chapter_lines[0]['nat'] if chapter_lines else chapter.file_name}</h1>\n"
    )
    ...
    for line_idx, line in enumerate(chapter_lines):
        ...
        if not skip_audio and not redo_pinyin:
            ...
            if len(emo_tag.split()) > 6:
                emo_tag = "Calm narrative"

            print(
                f"    [Audio] L{line_idx+1}/{len(chapter_lines)}: [{emo_tag}] {raw_text[:40]}..."
            )
            ...
            if torch.is_tensor(wavs[0]):
                ...
                if (
                    audio_t.numel() == 0
                    or torch.isnan(audio_t).any()
                    or torch.isinf(audio_t).any()
                ):
...
def run_export_stage(
    ...
    xhtml_path.write_text(
        f"<html><head><link rel='stylesheet' href='style/nav.css' type='text/css'/></head><body>{epub_html}</body></html>",
        encoding="utf-8",
    )
...
def process_novel(
    ...
    for f in all_files:
        ...
        if ch_num is not None and ch_num >= start_chapter:
            chapters.append(
                Chapter(novel_dir.name, f.name, f.read_text(encoding="utf-8"), ch_num)
            )
