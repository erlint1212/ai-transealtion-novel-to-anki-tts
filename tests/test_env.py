...
try:
    ...
    print_status(
        "Basic Packages", True, "ollama, genanki, rich, ebooklib imported successfully"
    )
...
try:
    ...
    print_status(
        "CUDA / GPU (NVIDIA-SMI)", False, "Could not communicate with NVIDIA GPU"
    )
...
try:
    ...
    print_status(
        "PyTorch CUDA",
        cuda_available,
        f"CUDA {cuda_version} detected. Using GPU: {gpu_name}",
    )
except ImportError:
...
try:
    ...
    print_status(
        "Flash-Attention 2", True, f"Version {flash_attn.__version__} installed"
    )
except ImportError as e:
    print_status(
        "Flash-Attention 2",
        False,
        "Not found. This usually fails if Python is not 3.10 or CUDA vars are missing.",
    )
...
try:
    ...
    print_status(
        ...
        (
            ...
            if model_found
            else f"Model not found. Available: {', '.join(model_names)}"
        ),
    )
